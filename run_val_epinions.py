...
class GraphRec(tf.keras.Model):
    ...
    def call(self, nodes_u, nodes_v, call_training):
        ...
        # tape.watch(self.r2e.trainable_variables)
        # tape.watch()
...
def train(model, device, train_loader, optimizer, epoch, best_rmse, best_mae, training=True):
    ...
    for i, data in enumerate(train_loader, 0):
        ...
        with tf.GradientTape(persistent=True) as tape:
            ...
            # tape.watch(model.r2e.trainable_variables)
...
def main():
    ...
    #2543731 making batch size 256 now, lr .15
...
#TODO: 
    ...
    ymaes = maes
    plt.plot(x, ymaes)
    plt.title("MAE per Epoch on Epinions Dataset")
    plt.xlabel("Epoch")
    plt.ylabel("Mean Absolute Error")
    plt.savefig("/home/ewang96/DLFinal/finplots/MAEs_epinions" + atime + ".jpg")
