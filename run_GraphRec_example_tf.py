...
class GraphRec(tf.keras.layers.Layer):
    ...
    def __init__(self, enc_u, enc_v_history, r2e):
        ...
        self.w_ur1 = tf.keras.layers.Dense(self.embed_dim, input_size=self.embed_dim)
        # self.w_ur2 = nn.Linear(self.embed_dim, self.embed_dim)
        self.w_ur2 = tf.keras.layers.Dense(self.embed_dim, input_size=self.embed_dim)
        # self.w_vr1 = nn.Linear(self.embed_dim, self.embed_dim)
        self.w_vr1 = tf.keras.layers.Dense(self.embed_dim, input_size=self.embed_dim)
        # self.w_vr2 = nn.Linear(self.embed_dim, self.embed_dim)
        self.w_vr2 = tf.keras.layers.Dense(self.embed_dim, input_size=self.embed_dim)
        # self.w_uv1 = nn.Linear(self.embed_dim * 2, self.embed_dim)
        self.w_uv1 = tf.keras.layers.Dense(self.embed_dim, input_size=2*self.embed_dim)
        # self.w_uv2 = nn.Linear(self.embed_dim, 16)
        self.w_uv2 = tf.keras.layers.Dense(16, input_size=self.embed_dim)
        # self.w_uv3 = nn.Linear(16, 1)
        self.w_uv3 = tf.keras.layers.Dense(1, input_size=16)
        self.r2e = r2e
    ...
    def call(self, nodes_u, nodes_v):
        ...
        x_u = tf.keras.layers.Dropout(x_u, rate=0.5, training=self.training)
        x_u = self.w_ur2(x_u)
        # x_v = F.relu(self.bn2(self.w_vr1(embeds_v)))
        x_v = tf.nn.relu(self.bn2(self.w_vr1(embeds_v)))
        # x_v = F.dropout(x_v, training=self.training)
        x_v = tf.keras.layers.Dropout(x_v, rate=0.5, training=self.training)
        x_v = self.w_vr2(x_v)
        ...
        x = tf.keras.layers.Dropout(x, rate=0.5, training=self.training)
    ...
    def loss(self, nodes_u, nodes_v, labels_list):
        scores = self.call(nodes_u, nodes_v)
...
def main():
    ...
    trainset = tf.data.Dataset.from_tensor_slices([tf.Tensor(train_u, dtype=tf.int64), tf.Tensor(train_v, dtype=tf.int64), tf.Tensor(train_r, dtype=tf.float32)])
    # testset = torch.utils.data.TensorDataset(torch.LongTensor(test_u), torch.LongTensor(test_v),
    #                                          torch.FloatTensor(test_r))
    testset = tf.data.Dataset.from_tensor_slices([tf.Tensor(test_u, dtype=tf.int64), tf.Tensor(test_v, dtype=tf.int64), tf.Tensor(test_r, dtype=tf.float32)])
    ...
    optimizer = tf.keras.optimizers.experimental.RMSprop(graphrec.parameters(), learning_rate=args.lr, epsilon=0.9)
